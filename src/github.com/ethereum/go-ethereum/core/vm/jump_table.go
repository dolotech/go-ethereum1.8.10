...
type operation struct {
	...
	halts   bool // indicates whether the operation should halt further execution表示操作是否停止进一步执行
	...
	returns bool // determines whether the operations sets the return data content 确定操作是否设置了返回数据内容
}
...
//jumptable. 是一个 [256]operation 的数据结构.
// 每个下标对应了一种指令, 使用operation来存储了指令对应的处理逻辑, gas消耗, 堆栈验证方法, memory使用的大小等功能.
